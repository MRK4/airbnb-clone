        ...
        <div className="relative">
            <div className="grid gap-2 grid-cols-[2fr_1fr] rounded-2xl overflow-hidden">
                <div>
                    {place.photos?.[0] && (
                        <div className="">
                            <Image onClick={() => setShowAllPhotos(true)} className="transition hover:brightness-105 cursor-pointer aspect-square object-cover w-full h-full" src={place.photos[0]} alt="" />
                        </div>
                    )}
                </div>
                <div className="grid">
                    {place.photos?.[1] && (
                        <Image onClick={() => setShowAllPhotos(true)} className="transition hover:brightness-105 cursor-pointer aspect-square object-cover w-full h-full" src={place.photos[1]} alt="" />
                    )}
                    <div className="overflow-hidden">
                        {place.photos?.[2] && (
                            <Image onClick={() => setShowAllPhotos(true)} className="transition hover:brightness-105 cursor-pointer relative top-2 aspect-square object-cover w-full h-full" src={place.photos[2]} alt="" />
                        )}
                    </div>
                </div>
            </div>
